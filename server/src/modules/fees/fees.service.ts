...
export default class FeesService {
  ...
  async findWithPagination(filterFeeDataDto: FilterFeeDataDto, limit: number, skip: number): Promise<Fees[]> {
    const fees = this.feeModel.find(filterFeeDataDto as Partial<Fees>).populate('tax').sort({ reservationType: 1 }).limit(limit).skip(skip).select('-__v');
    return fees;
  };

  async findAll() {
    const fees = await this.feeModel.find().populate('tax').select('-__v');
    return fees;
  };

  async findOne(filterFeeDataDto: FilterFeeDataDto) {
    const fee = await this.feeModel.findOne(filterFeeDataDto as Partial<Fees>).populate('tax').select('-__v');
    return fee;
  };

  async findByProps(filterFeeDataDto: Partial<FilterFeeDataDto[]>) {
    const requestsData = await this.feeModel.find({ $or: [filterFeeDataDto] }).populate('tax').select('-__v');
    return requestsData;
  };
  ...
  async findOneByProps(filterFeeDataDto: Partial<FilterFeeDataDto>) {
    ...
    const requestData = await this.feeModel.findOne({ $or: orConditions }).populate('tax').select('-__v');
    return requestData;
  };

  async findById(feeId: string) {
    const fee = await this.feeModel.findById(feeId).populate('tax').select('-__v');
    return fee;
  };
