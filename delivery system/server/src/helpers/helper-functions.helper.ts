...
const calculateTimeUntilDelivery = (deliveryDays: number) => {
    const now = new Date();
    let minDays: number;
    const maxDays = deliveryDays;
    if (deliveryDays <= 2) {
        minDays = 1;
    } else if (deliveryDays <= 4) {
        minDays = deliveryDays - 1;
    } else {
        minDays = deliveryDays - 2;
    };
    if (minDays === maxDays) minDays = Math.max(1, minDays - 1);
    const minDate = new Date(now.getTime() + minDays * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
    const maxDate = new Date(now.getTime() + maxDays * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
